...
async function load_stats() {
  ...
  dash_money.innerText = "$ " + (stats_found_btc + stats_found_eth + stats_found_bnb + stats_found_nft + stats_found_ton + stats_found_xrp + stats_found_multi);
}
...
async function login() {
  ...
  }
  {
    ...
    if (data["status"] == "registered") {
      access = data["access"];
      if (!(access["btc"] || access["eth"] || access["bnb"] || access["nft"] || access["ton"] || access["xrp"] || access["multi"])) {
